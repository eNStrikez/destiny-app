...
var addPlayer = function(data){
	...
	$('ul').append('<li><div id="player">' + name + type + '<button class="del_button" id="del_button' + id + '"><ion-icon id="icon" name="close"></ion-icon></button></div></li>');
	initButton();
...
var addItem = function(data, currentID){
	...
	if(data.itemCategoryHashes.includes(1)){
		$('#del_button' + currentID).before('<div id="item"><p class="data" id="data' + id + '"> <a href="weapon.html?hash=' + data.hash + '">' + icon + '</a> <strong>' + name + '</strong>    <small><i>'+ desc + '</i></small>' + '</p></div>');
	}
	initData();
};
